...
class Action
{
    ...
    if(isset($this->definition["FIELDS"]))
    {
        ...
        if ($actionResult->isOk()) {
            ...
            if ($def["ROLE"] != "SEARCH") {
                ...
                if (!$this->destModel) {
                    ...
                    if ($def["MODEL"]) {
                        $this->destModel = \lib\model\BaseModel::getModelInstance($def["MODEL"]);
                        if ($keys) {
        ...
        if ($actionResult->isOk()) {
            ...
            if ($def["ROLE"] != "SEARCH" && $def["ROLE"]!="Delete") {
                try {
        ...
        if ($actionResult->isOk()) {
            ...
            try {
                ...
                {
                    $actionResult->setModel($this->onSuccess($this->destModel, $user));
                }
            } catch (\Exception $e) {
                $actionResult->addGlobalError($e);
                return false;
