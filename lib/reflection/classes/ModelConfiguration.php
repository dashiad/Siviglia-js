...
class ModelConfiguration extends \lib\reflection\classes\ConfigurationFile
{
    var $objName;    
    var $config;
    var $definition;
    function __construct($parentModel)
    {
        ...
        $className=$parentModel->objectName->className;
        $layer=$parentModel->objectName->layer;
        $path=PROJECTPATH."/".$layer."/objects/".$className."/config.php";
        $namespace=$layer.'\\'.$className;
        ConfigurationFile::__construct($path,$namespace);       
    }
