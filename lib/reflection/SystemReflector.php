<?php
   ...
   class ReflectorFactory
   {
        ...
        static function loadFactory()
        {
            ...
            $modelPath=\lib\Paths::getModelPath();
            $dirModel=opendir($modelPath);

            while($curLayer=readdir($dirModel))
            {
                echo $curLayer."<br>";
                if($curLayer[0]==".")
                    continue;
                if(!is_dir($modelPath."/".$curLayer))
                    continue;
                $dir=\lib\Paths::getModelPath()."/$curLayer/objects";
