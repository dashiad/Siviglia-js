        ...
        if (isset($this->resources[$phase])) {
            ...
            foreach ($res as $key => $value) {
                ...
                for ($j = 0; $j < count($value); $j++) {
                    ...
                    // Si el primer elemento no es nulo, es que habia codigo escrito inline.
                    if ($cur[0] !== null) {
                        // Si NO estamos en desarrollo, ese texto habra que meterlo en el fichero del bundle (Global.js,etc)
                        // Asi que lo añadimos al texto comun.
                        if (!defined("DEVELOPMENT") || DEVELOPMENT == 0) {
                            // Codigo inline;
                            $text .= $cur[0];
                        } else {
                            // Si estamos en desarrollo, el código se va a meter inline, dentro de la propia salida,
                            // asi que se mete en tags script inline
                            switch ($key) {
                                case "SCRIPT":
                                    {
                ...
                if ($key != "HTML" && $text!=="") {
                    ...
                    // Si al final hay algun contenido que debe ir al fichero de bundle:
...
class DEPENDENCY extends Plugin
{
    ...
    function parse_WIDGET($spec, $bundle)
    {
        if (isset($spec["MODEL"])) {
            ...
            if (!is_file($srcFile)) {
                die("Widget no encontrado : Objeto:" . $spec["MODEL"][0] . " Widget:" . $spec["NAME"][0]);
            }
        } else {
