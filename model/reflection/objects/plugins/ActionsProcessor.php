...
class ActionsProcessor extends \model\reflection\base\SystemPlugin
{
    ...
    function generateActions($objName, $objModel, $layer,$isadmin)
    {
        ...
        if($objModel->getExtendedModelName())
        {
            ...
            $parentInstance=\model\reflection\ReflectorFactory::getModel($extendedModel);
        ...
        $defaultActions=array("NAMES"=>array("DeleteAction","EditAction"),
                              "PERMISSIONS"=>array(
                                  \model\reflection\Permissions\PermissionRequirementsDefinition::create(array_merge($modifyPermission, array(array("MODEL"=>$objName,"PERMISSION"=>"delete")))),
                                  \model\reflection\Permissions\PermissionRequirementsDefinition::create(array_merge($modifyPermission, array(array("MODEL"=>$objName,"PERMISSION"=>($isadmin?"adminEdit":"edit")))))
                                  ),
        ...
        if(!$objModel->getSubTypes())
        {
            ...
            $defaultActions["PERMISSIONS"][]=\model\reflection\Permissions\PermissionRequirementsDefinition::create(array(array("MODEL"=>$objName,"PERMISSION"=>($isadmin?"adminCreate":"create"))));
        ...
        if($role=="MULTIPLE_RELATION")
        {
            ...
            foreach($mrelfields as $curf)
            {
                 $fake=new \model\reflection\Model\Alias\FakeRelationship($name,$objModel);
