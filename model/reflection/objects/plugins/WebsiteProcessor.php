...
class WebsiteProcessor extends \model\reflection\base\SystemPlugin
{
    ...
    function BUILD_URLS($level)
    {
      ...
      if($level!=2)return;
      $this->paths=array();
      $this->configurationFile=new \model\reflection\base\ConfigurationFile("Config", "", "\html",WEBROOT."/config.php");
...
/*       printPhase("Generando Paths");
       ...
       $pathHandler=new \model\reflection\Html\UrlPathDefinition($this->paths);
       $pathHandler->save();
    ...
    function saveUrlTree()
    {
       ...
       $pathHandler=new \model\reflection\Html\UrlPathDefinition($ftree["SUBPAGES"]);
       $pathHandler->save();
       $this->configurationFile->save();
    }
