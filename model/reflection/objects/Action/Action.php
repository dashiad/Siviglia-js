...
class Action extends \model\reflection\base\ConfiguredObject
{
    ...
    {
        $this->name=$name;
        parent::__construct($name, $parentModel,'\actions','actions','actions','\lib\action\Action');
    }
    ...
    function initialize($definition=null)
    {
        ...
        if(isset($this->definition["PERMISSIONS"]))
            $this->permissionsObj=new \model\reflection\Permissions\PermissionRequirementsDefinition($this->definition["PERMISSIONS"]);
        else
    ...
    function create($type, $indexFields,$requiredFields, $optionalFields,$remoteModel=null,$isadmin=false,$relationName="",$paths=array())
    {
        ...
        //$perm = \model\reflection\Permissions\PermissionRequirementsDefinition::create(array_merge($modifyPermission, array(array("MODEL"=>$objName,"PERMISSION"=>"edit"))));
        $pIndex=0;
        ...
        $permissionsObj = \model\reflection\Permissions\PermissionRequirementsDefinition::create($basePerm);
        ...
        foreach($req as $key=>$value)
        {
            ...
            $classInstance=\model\reflection\ReflectorFactory::getModel($fieldClass);
            ...
            if($value->__isRelation())
            {
               ...
               $def["FIELDS"][$key]["DATASOURCE"]=array("MODEL"=>$target,"NAME"=>"FullList","PARAMS"=>array());
            }
        ...
        foreach($opt as $key=>$value)
        {
            ...
            //if(!is_a($value,'\model\reflection\Model\Alias\FakeRelationship'))
            //{                
                //$classInstance=\model\reflection\ReflectorFactory::getModel($fieldClass);
    ...
    {
        return io($this->definition,"TARGET_RELATION",null);
    }
    ...
    function saveModelMethods()
    {
        $def=$this->getDefinition();
                    ...
                    "PARAMS"=>array(
                        "params"=>array(
                            "COMMENT"=>" Parameters received,as a BaseTypedObject.\nIts fields are:\n".(isset($def["INDEXES"])?"keys: ".implode(",",array_keys($def["INDEXES"]))."\n":"").
                                (isset($def["FIELDS"])?"fields: ".implode(",",array_keys($def["FIELDS"])):"")
            ...
            $this->addMethod(array(
                    ...
                    "PARAMS"=>array(
                        "keys"=>array("COMMENT"=>" Keys received"),
                        "params"=>array(
