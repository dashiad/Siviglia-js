...
class TypeSwitcherException extends \model\reflection\Types\meta\BaseTypeException
{
...
class TypeSwitcher extends BaseContainer
{
    ...
    function getValue()
    {
        if($this->valueSet==false || $this->currentType==null)
                return null;

        if(!$this->value->hasOwnValue())
            return null;
    ...
    function __rawSet($value)
    {
        ...
        if(!isset($value[$this->type_field]))
            $type=$this->implicit_type;
        else
            $type=$value[$this->type_field];
        $typeInstance=$this->getTypeInstance($type);
        if($this->content_field!==null)
            $typeInstance->__rawSet($value[$this->content_field]);
        else
            $typeInstance->__rawSet($value);
        $this->value=$typeInstance;
    ...
    function getTypeInstance($type)
    {
        ...
        return \model\reflection\Types\meta\TypeFactory::getType($this,$def);
    }
}
