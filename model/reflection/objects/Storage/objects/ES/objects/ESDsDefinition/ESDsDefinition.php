    ...
    static $connections;
        ...
        function generateFromQuery($model,$dsName,$query)
        {
            ...
            */
            ...
            $myDs=new ESDsDefinition($model,$dsName,$ds);
        ...
        function create()
        {
            ...
            if($meta)
            {
               foreach($meta as $metaK=>$metaD)
               {
                   if($metaD["FIELD"])
                   {
                       $cModel=\model\reflection\ReflectorFactory::getModel($metaD["MODEL"]);
                       $field=$cModel->getField($metaD["FIELD"]);
                   }
                   if(isset($metaD["REFERENCES"]))
                   {
                       $cModel=\model\reflection\ReflectorFactory::getModel($metaD["REFERENCES"]["MODEL"]);
            ...
            if(isset($def["PAGESIZE"]) && $pagerVariable!=null)
            {
        ...
        static function createDataSourceFromQuery($curModel, $queryName,$queryDef)
        {
            ...
            $relMap = \model\reflection\ReflectorFactory::getRelationMap();
            $q = ESDsDefinition::getQuery($tables,array(),$parameters,$relMap["distances"],$relMap);
            ...
            if(!$ds || $ds->mustRebuild())
            {
                ...
                {
                    $cModel=\model\reflection\ReflectorFactory::getModel($curTable);
        ...
        function createMxNDsDefinition($fromInverse)
        {
            ...
            $modelDef=\model\reflection\ReflectorFactory::getModel($srcModelName);
        ...
        public function discoverFields()
        {
            ...
            $minDef["TABLE"]=isset($definition["DEFINITION"]["TABLE"])?$definition["DEFINITION"]["TABLE"]:null;
            ...
            if(!isset(ESDsDefinition::$connections[$key]))
            {
                ESDsDefinition::$connections[$key]=mysqli_connect($d["host"],$d["user"],$d["password"],$d["database"]["NAME"]);
            }
            $connRes=ESDsDefinition::$connections[$key];
            ...
            for($k=0;$k<$nFields;$k++)
            {
                ...
                if($fTable==$parentTable) // Puede ser un campo del modelo.En ese caso, obtenemos la informacion del campo.
                {
                    ...
                    // incluido.
                    if(isset($fieldColumns[$fName]))
                    {
                        if(!isset($returnedFields[$sourceField]))
                ...
                if( !preg_match("/^[a-zA-Z][a-zA-Z0-9_]*$/",$fName) )
                    ...
                    // Vemos si tiene alguna tabla.
                        ...
                        $returnedFields[$fName]=array("TYPE"=>$typeConversions[$fieldType],"MAXLENGTH"=>$maxlength);
    ...
    function findTableField($tableName,$field)
    {
        ...
        static function expr2Query($expr,$definition,$lastTable,$joinType=" INNER", $mode=0)
        {
            ...
            for($k=1;$k<count($parts);$k++)
            {
                ...
                if(count($p2)==2)
                {
                    $model=\model\reflection\ReflectorFactory::getModel($p2[0]);
        ...
        static function getQuery($valueTables,$conditionFields,$parameterFields,$schema,$definition)
        {
            ...
            foreach($distances as $name=>$distData)
            {
                ...
                foreach($targetTables as $value)
                {
                    ...
                    $results=ESDsDefinition::expr2Query($schema[1][$curTable][$value],$definition,$lastTable,$joinType);
                    $q.=$results["query"];
           ...
           for($k=0;$k<count($valueTables);$k++)
           {
               ...
               $model=\model\reflection\ReflectorFactory::getModel($ttable);
               if(!$model)
