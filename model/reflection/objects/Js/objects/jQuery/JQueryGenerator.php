...
class JQueryGenerator
{
    ...
    function generate()
    {
        $modelName=$this->normalizeJsName($this->model);
        $text=<<<CLASS
...
CLASSCODE;
    ...
    function getFormInput($name,$def,$form,& $requires)
    {
        ...
        $paramData="";
        if(isset($fDef["INPUTS"][$name]) && isset($fDef["INPUTS"][$name]["PARAMS"])) {
    ...
    function generateDataSourceView($name,$datasource)
    {
        ...
        foreach($fields as $key=>$value)
        {
            ...
            $dojoDef[$keyS]=array("name"=>$key,"display"=>true,"order"=>$k,"label"=>isset($value['LABEL'])?'[@L]'.$value['LABEL'].'[#]':'[@L]'.$key.'[#]');
            $dojoFuncs[$keyS]="\t\t\t\t/*,show_".$keyS.":function(object, value, node, options){ node.innerHTML=value;}*/";
            $k++;
        }
        $fullDojoDef=json_encode($dojoDef, JSON_UNESCAPED_UNICODE);
        ...
        foreach($fields as $key=>$value) {
            if (isset($value['MODEL'])) {
                $tModel=\model\reflection\ReflectorFactory::getModel($value["MODEL"]);
...
TEMPLATE;
    ...
    function generateDataSourceForm($datasource,& $mappings)
    {
            ...
            if(isset($value["MODEL"]))
            {
                ...
                if($field->isRelation())
                {
                    ...
                    foreach($remFields as $keyV=>$valueV)
                    {
                        ...
                        $curName=$key."_".$n;
                        $mappings[$key][$curName]=array("LABEL"=>$f->getLabel(),"HTML"=>'<span data-siviglia-name="'.$curName.'" data-siviglia-input-type="'.$inputType.'"  data-siviglia-input-params=\'{"LABEL":["'.$keyV.'"],"VALUE":"'.$pointedField.'","NULL_RELATION":[0],"PRE_OPTIONS":["Select an option"],"DATASOURCE":{"NAME":"FullList","MODEL":"'.$remoteNormalized.'"}}\' data-siviglia-input-definition=\'{"DEFAULT":"NULL","FIELDS":{"'.$key.'":"'.$pointedField.'"},"MODEL":"'.$remoteNormalized.'","TYPE":"'.$inputType.'","MULTIPLICITY":"1:N","ROLE":"HAS_ONE","CARDINALITY":'.$cardinality.'}\'></span>');
        ...
        if(!isset($fields) && empty($mappings))
        {
            return '';
        }
...
TEMPLATE;
    ...
    function getDataSourceFormInput($name,$def)
    {
        ...
        if($def["MODEL"])
        {
            $model=\model\reflection\ReflectorFactory::getModel($def["MODEL"]);
        ...
        <div data-dojo-type="dijit/Dialog" data-dojo-id="saveSelectionDialog" data-dojo-attach-point="saveSelectionDialog" title="Guardar selección">
            <div  data-dojo-type="dijit/layout/ContentPane" data-dojo-id="saveSelectionDialogPane" title="Pane" doLayout="false" style="width: 360px; position: relative; height: 160px;">
                <form data-dojo-type="dijit/form/Form" data-dojo-attach-point="saveSelectionForm">
                    <div>
                        <div style="height: 30px;">
                            <div style="float: left;width: 124px;">[@L]Objeto[#]</div>
                            <div style="float: left;margin-left:10px;" data-dojo-attach-point="objectSelectorWrapper"></div>
                        </div>
                        <div style="height: 30px;">
                            <div style="float: left;width: 124px;">
                                <input data-dojo-type="dijit/form/RadioButton" name="groupNameSelector" id="groupNameSelector1" checked value="1" /><label for="groupNameSelector1" style="margin-left:10px;">[@L]Nuevo grupo[#]</label>
                            </div>
                        ...
                        <div style="height: 30px;">
                            <div style="float: left;width: 124px;">
                                <input data-dojo-type="dijit/form/RadioButton" name="groupNameSelector" value="2" id="groupNameSelector2" /><label for="groupNameSelector2" style="margin-left:10px;">[@L]Grupo existente[#]</label>
                            </div>
                            <div style="float: left;margin-left:10px;" data-dojo-attach-point="groupSelectorWrapper"></div>
                        </div>
                        <div style="height: 30px;">
                            <div style="float: left;width: 124px;">[@L]Nombre de subgrupo[#]</div>
                ...
                <div style="position: absolute; z-index: 900; bottom: 5px; right: 5px;">
                    <input type="button" data-dojo-type="dijit/form/Button" intermediateChanges="false" label="[@L]Cancelar[#]"
                           iconClass="dijitNoIcon" onClick="saveSelectionDialog.onCancel();">
                    <input type="button" data-dojo-type="dijit/form/Button" intermediateChanges="false" label="[@L]Aceptar[#]" iconClass="dijitNoIcon"
...
EOL;
    ...
    {
        $text = <<<EOL
        <div data-dojo-type="dijit/Dialog" data-dojo-id="chooseSelectionDialog" data-dojo-attach-point="chooseSelectionDialog" title="[@L]Abrir selección[#]">
                ...
                <form data-dojo-type="dijit/form/Form" data-dojo-attach-point="chooseSelectionForm">
                    <div>
                        <div style="height: 30px;display:none;" data-dojo-attach-point="CSdataGroupMemberSelectorContainer">
                            <div style="float: left;width: 124px;">[@L]Selección guardada[#]</div>
                        ...
                        <div style="height: 30px;display:none;" data-dojo-attach-point="CSdataGroupMemberSelectorEmpty">
                            [@L]No se encuentra ninguna selección guardada coincidente con la selección actual[#]
                        </div>
                    </div>
                </form>
                <div style="position: absolute; z-index: 900; bottom: 5px; right: 5px;">
...
EOL;
    ...
    function regenerateTranslations($dsName,$datasource)
    {
        $destFile=$this->model->objectName->getDestinationFile("js/dojo/views/".$dsName.".js");
        if (!file_exists($destFile)) {
            throw new \RuntimeException('Destination file do not exists');
        }

        $code = file_get_contents($destFile);

        //Sacamos la información de fieldDefinition para regenerarla
        $dsDef=$datasource->getDefinition();
        $fields=$dsDef["FIELDS"];
        $k=0;
        $dojoDef=array();
        ...
        {
            $keyS=str_replace("/","_",$key);
            $dojoDef[$keyS]=array("name"=>$key,"display"=>true,"order"=>$k,"label"=>$value['LABEL']?'[@L]'.$value['LABEL'].'[#]':'[@L]'.$key.'[#]');
            $k++;
        }
        $fullDojoDef=json_encode($dojoDef, JSON_UNESCAPED_UNICODE);

        //Sustituimos en código la definición antigua por la nueva
        $code = preg_replace('#fieldDefinition:.*\n#', "fieldDefinition:".$fullDojoDef.",\n", $code);

        $targetDir=dirname($destFile);
        file_put_contents($targetDir."/".$dsName.".js",$code);
    }
}
