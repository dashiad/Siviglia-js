...
include_once(PROJECTPATH."/model/reflection/objects/DataSource/DataSourceParameterDefinition.php");
include_once(PROJECTPATH."/model/reflection/objects/DataSource/DataSourceIncludeDefinition.php");
include_once(PROJECTPATH."/model/reflection/objects/Permissions/PermissionRequirementsDefinition.php");
class DataSource extends \model\reflection\base\ConfiguredObject
{       
    ...
    {        
        ...
        $this->role=io($this->definition,"ROLE",null);
        ...
        if(isset($this->definition["INCLUDE"]))
        {
            ...
            {
                $this->includes[$key]=new \model\reflection\DataSource\DataSourceIncludeDefinition($value);
            }            
        }                
    ...
    function loadParams()
    {
        ...
        foreach($this->definition["PARAMS"] as $key=>$value)
            $this->params[$key]=new \model\reflection\DataSource\DataSourceParameterDefinition($key,$value);
    }
    function getOwnershipField()
    {
        return $this->parentModel->getOwnershipField();
    }
    function getRole()
    {
    ...
    {    
        ...
        {
            $paramType=\model\reflection\Model\Type\ModelReferenceType::create($this->parentModel->objectName->getNormalizedName(),$key);
        ...
        foreach($filterFields as $key=>$value)
        {
            ...
            if(is_object($value))
            {
                $paramType=\model\reflection\Model\Type\ModelReferenceType::create($this->parentModel->objectName->getNamespaced(),$key);
        ...
        */
        ...
        foreach($relationFields as $key=>$value)
        {
           ...
           while(isset($usedNames[$subDsName.$j]))$j++;
               $usedNames[$subDsName.$j]=1;
           $includeName=$j?$subDsName.$j:$subDsName;
           $instance=\model\reflection\DataSource\DataSourceIncludeDefinition::create($key,$value,($dsDefinition["IS_ADMIN"]?"AdminFullList":"FullList"),"LEFT");
        ...
        if(strtolower($role)=="view")
        {
            ...
            foreach($aliases as $key=>$value)
            {
                if($value->__isRelation())
                {
                       ...
                       $instance=\model\reflection\DataSource\DataSourceIncludeDefinition::createFromAlias($key,$value,$key,"INNER");
       ...
       $perms=\model\reflection\Permissions\PermissionRequirementsDefinition::create($permissions);
...
*/
        $def["INCLUDE"][$firstField->parentModel->objectName->getNormalizedName()]=array(
                          "MODEL"=>$firstField->parentModel->objectName->getNormalizedName(),
                          "DATASOURCE"=>"View",
                          "JOINTYPE"=>$including,
    ...
    function createFromMxNRelation($inverseRelationField,$role,$including,$isadmin,$permissions)
    {
        ...
        */
        ...
        {
            $paramType=\model\reflection\Model\Type\ModelReferenceType::create($srcClass,$keyF);
         ...
         foreach($def["FIELDS"] as $key=>$value)
         {
             ...
             $target=\model\reflection\ReflectorFactory::getModel($model);
    ...
    function getWidget($outputType)
    {
        if(!isset($this->outputWidgets[$outputType]))
        {
            ...
            $generator=new $className($this->getName(),$this->parentModel,$this);
    ...
    {
        include_once(__DIR__."/DataSourceMetadata.php");
        return new \model\reflection\DataSource\DataSourceMetadata($objectName,$dsName);
    }
}
?>
